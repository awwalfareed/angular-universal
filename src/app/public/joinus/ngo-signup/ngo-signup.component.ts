...
export class NgoSignupComponent implements OnInit {
  ...
  constructor(
   ...
   // get localstorage data
    if (sessionStorage.getItem('nsd')) {
      ...
      this.model.selectedPhoneCode = this.Local?.selectedPhoneCode;
  ...
  ngoSignup({ value, valid }: { value: any, valid: boolean }) {
    if (valid === true) {
      ...
      if (this.isFounder) {
        localStorage.setItem('cp', btoa(value.contactPerson));
      }
  ...
  // domainFocus(): void {
  //  document.querySelector('#ngo-sub-domain')[0].focus();
  // }
}
